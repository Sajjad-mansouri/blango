...
class PostApiTestCase(TestCase):
	...
	def test_post_list(self):
		...
		for post_dict in data:
			...
			self.assertTrue(
			post_dict['author'].endswith(f'api/v1/users/{post_object.author.email}')
			)
			self.assertEqual(
			post_object.published_at,datetime.strptime(post_dict['published_at'],"%Y-%m-%dT%H:%M:%S.%fZ").replace(tzinfo=UTC))
	...
	def test_post_create(self):
		...
		post=Post.objects.get(pk=resp_id)
